# Publica lotes NOT PII en datasets latest, daily y monthly.
...
from atlanticus.data_producers.notpii.models import NotPiiBatch
...
class NotPiiMaterializer:
    def __init__(
        ...
        *,
        ...
        if not isinstance(runtime, DatasetRuntime):
            raise NotPiiDataProducerConfigurationError('runtime must be a DatasetRuntime')
    ...
    def publish(self, batch: NotPiiBatch) -> tuple[DatasetPublicationResult, ...]:
        ...
        for materialization in _MATERIALIZATION_ORDER:
            definitions = tuple(
                item for item in self._definitions if materialization in item.materializations
            )
            if not definitions:
                continue
...
def _build_dataset_definition(
    *,
    ...
    return DatasetDefinition(
        key=DatasetKey(namespace=dataset_namespace, name=extraction_mode.value),
