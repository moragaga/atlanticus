# Espejo comentado del productor PI reutilizable: materialization.py
...
from atlanticus.data_producers.pi.errors import PiDataProducerMaterializationError
...
class PiDataProducerMaterializer:
    def __init__(
        self,
        *,
        ...
        if not isinstance(catalog.source, PiWebApiSource):
            raise PiDataProducerMaterializationError('catalog source must be PiWebApiSource')
        if catalog.source.interpolation_seconds is None:
            raise PiDataProducerMaterializationError('catalog must define interpolation_seconds')
        if (
            not isinstance(dataset_namespace, tuple)
            or not dataset_namespace
            or any(not isinstance(item, str) or not item.strip() for item in dataset_namespace)
        ):
            raise ValueError('dataset_namespace must be a non-empty tuple of text values')
        self._runtime = runtime
        self._catalog = catalog
        self._dataset_namespace = tuple(item.strip() for item in dataset_namespace)
        ...
        self._datasets = {
            mode: _build_dataset_definition(
                mode=mode,
...
def _build_dataset_definition(
    *,
    ...
    return DatasetDefinition(
        key=DatasetKey(namespace=dataset_namespace, name=mode.value),
...
def _group_slots(
    ...
    for slot in slots:
        if materialization is PiMaterialization.DAILY:
            ...
            raise PiDataProducerMaterializationError('unsupported partitioned materialization')
