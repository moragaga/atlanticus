...
def test_active_partition_preserves_retired_columns_and_new_partition_uses_current_catalog(
    ...
    initial = PiDataProducerMaterializer(runtime=runtime, catalog=initial_catalog)
    ...
    current = PiDataProducerMaterializer(runtime=runtime, catalog=current_catalog)
...
def test_active_partition_rejects_type_change_for_existing_alias(tmp_path) -> None:
    ...
    PiDataProducerMaterializer(runtime=runtime, catalog=initial_catalog).publish(
        window=_window(slot, slot),
        acquisition=PiAcquisitionResult(
            interpolated=(PiSample('TAG_A', slot, 1),),
            recorded=(),
    ...
    changed = PiDataProducerMaterializer(runtime=runtime, catalog=changed_catalog)
    next_slot = datetime(2026, 8, 15, 10, 0, 10, tzinfo=UTC)

    with pytest.raises(PiDataProducerMaterializationError, match='column type is incompatible: a'):
        changed.publish(
            window=_window(next_slot, next_slot),
            acquisition=PiAcquisitionResult(
