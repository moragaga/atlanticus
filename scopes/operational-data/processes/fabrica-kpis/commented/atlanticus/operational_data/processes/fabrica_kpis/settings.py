...
from atlanticus.operational_data.processes.fabrica_kpis.errors import (
    FabricaProcessConfigurationError,
)

STORAGE_SUFFIX = 'FABRICA_KPIS'


@dataclass(frozen=True, slots=True)
# Requiere solo el SAS y la configuración pertenecientes a este proceso.
class FabricaProcessSettings:
    ...
    def from_configuration(cls, configuration: ResolvedConfiguration) -> FabricaProcessSettings:
        ...
        if url_token and configured_token and url_token != configured_token:
            raise FabricaProcessConfigurationError(f'{url_key} and {token_key} contain different SAS tokens')
        token = configured_token or url_token
        if not token:
            raise FabricaProcessConfigurationError(f'{token_key} is required when {url_key} has no query')
        try:
            settings = StorageSettings(credential=StorageSasCredential(
                account_url=urlunsplit((parsed.scheme, parsed.netloc, '', '', '')),
                sas_token=token, allow_insecure_http=parsed.scheme == 'http',
            ))
        ...
        try:
            ...
            raise FabricaProcessConfigurationError('FABRICA_IDLE_SECONDS must be a positive integer') from None
        if idle <= 0:
            raise FabricaProcessConfigurationError('FABRICA_IDLE_SECONDS must be a positive integer')
        return cls(connection=FabricaStorageConnection(settings=settings, container_name=container[0]), idle_seconds=idle)
...
def configuration_specs() -> tuple[ConfigurationVariableSpec, ...]:
    return (
        ...
        ConfigurationVariableSpec(key=f'STORAGE_ACCOUNT_SAS_TOKEN_{STORAGE_SUFFIX}', required=False, sensitive=True),
