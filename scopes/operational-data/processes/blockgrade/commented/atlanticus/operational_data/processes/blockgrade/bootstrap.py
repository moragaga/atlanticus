...
def load_configuration(
    *,
    ...
    try:
        ...
        raise BlockgradeProcessConfigurationError(str(error)) from error
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise BlockgradeProcessConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise BlockgradeProcessConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise BlockgradeProcessConfigurationError('VOLUMEN_PATH must be an absolute path')
