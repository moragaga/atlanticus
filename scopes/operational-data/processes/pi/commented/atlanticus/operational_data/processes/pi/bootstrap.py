...
from atlanticus.operational_data.processes.pi.composition import build_composition
...
def load_configuration(
    *,
    ...
    secret_entries = tuple(item for item in manifest.entries if item.exists_in_key_vault)
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise PiWebApiProcessConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise PiWebApiProcessConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(
    ...
    if not configured_path.is_absolute():
        raise PiWebApiProcessConfigurationError('VOLUMEN_PATH must be an absolute path')
