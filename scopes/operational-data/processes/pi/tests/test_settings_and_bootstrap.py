...
from atlanticus.operational_data.processes.pi.bootstrap import load_configuration
from atlanticus.operational_data.processes.pi.errors import PiWebApiProcessConfigurationError
from atlanticus.operational_data.processes.pi.settings import (
...
def test_local_bootstrap_resolves_process_values_without_secret_files(tmp_path: Path) -> None:
    ...
    assert configuration.require('APPLICATION') == 'operational-data-pi'
...
def test_deployed_bootstrap_derives_key_vault_from_company_environment_and_product(
    ...
    import atlanticus.operational_data.processes.pi.bootstrap as bootstrap_module
    ...
    assert settings.company_abrev == 'cmp'
    assert str(settings.environment) == 'prd'
    assert settings.product_abrev == 'ops'
    assert settings.vault_name == 'cmp-prd-kv-ops'
    assert configuration.require('PI_WEB_API_USERNAME') == 'domain\\user'
