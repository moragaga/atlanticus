# El bootstrap resuelve configuración local o secretos de Key Vault antes de componer el proceso.
# Remanentes exige un VOLUMEN_PATH absoluto para que state y datasets tengan una raíz inequívoca.
...
from atlanticus.operational_data.processes.remanentes.composition import build_composition
from atlanticus.operational_data.processes.remanentes.errors import (
    RemanentesProcessConfigurationError,
)
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise RemanentesProcessConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise RemanentesProcessConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise RemanentesProcessConfigurationError('VOLUMEN_PATH must be an absolute path')
