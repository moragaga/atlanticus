...
def test_commented_mirror_preserves_productive_behavior() -> None:
    ...
    productive = root / 'src' / 'atlanticus' / 'operational_data' / 'processes' / 'notpii'
    commented = root / 'commented' / 'atlanticus' / 'operational_data' / 'processes' / 'notpii'
    ...
    for relative_path in sorted(productive_files):
        productive_ast = ast.dump(
            ast.parse((productive / relative_path).read_text()), include_attributes=False
        )
        commented_ast = ast.dump(
            ast.parse((commented / relative_path).read_text()), include_attributes=False
        )
