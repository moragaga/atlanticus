...
from atlanticus.operational_data.processes.notpii.catalog import (
    active_extraction_modes,
    build_catalog,
)
from atlanticus.operational_data.processes.notpii.composition import build_composition
...
def load_configuration(
    *,
    ...
    active_modes: tuple[PiExtractionMode, ...],
) -> ResolvedConfiguration:
    source_values = os.environ if environ is None else environ
    root = Path(process_root)
    specs = configuration_specs(active_modes=active_modes)
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise NotPiiProcessConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise NotPiiProcessConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise NotPiiProcessConfigurationError('VOLUMEN_PATH must be an absolute path')
...
def run(
    *,
    ...
    active_modes = active_extraction_modes(catalog)
    configuration = load_configuration(
        process_root=root,
        environ=source_values,
        active_modes=active_modes,
    )
    return build_composition(configuration=configuration, catalog=catalog).execute(argv=argv)
