...
def test_process_is_owned_by_operational_data_workspace() -> None:
    ...
    assert project['project']['name'] == 'atlanticus-operational-data-fabrica-process'
    ...
    assert project['project']['scripts'] == {
        'operational-data-fabrica': 'atlanticus.operational_data.processes.fabrica.bootstrap:main'
    }
    assert project['tool']['atlanticus']['container']['command'] == 'operational-data-fabrica'
    assert 'tool' in workspace
    assert 'processes/fabrica' in workspace['tool']['uv']['workspace']['members']
    assert workspace['tool']['uv']['sources']['atlanticus-operational-data-fabrica-process'] == {
        'workspace': True
    }
...
def test_process_uses_conventional_configuration_root() -> None:
    root = Path(__file__).resolve().parents[1]
    bootstrap = (
        root / 'src' / 'atlanticus' / 'operational_data' / 'processes' / 'fabrica' / 'bootstrap.py'
