...
from atlanticus.operational_data.calendar import (
    MINE_CALENDAR,
    ShiftIdTurn,
...
from atlanticus.operational_data.core import ShiftScope, ShiftSelection, normalize_utc_second
...
class MineShiftResolver:
    def resolve(
        ...
        *,
        ...
        current = get_current_shift_id_turn(value=value)
        if selection.scope is ShiftScope.CURRENT:
            return (current,)
        if selection.scope is ShiftScope.PREVIOUS:
            return (get_previous_shift_id_turn(value=value),)
