# Espejo comentado: Orquesta evaluación, lifecycle, prioridad, inputs operacionales y commit.
...
class AlarmOperationalCycle:
    ...
    def _evaluate_entry(
        ...
        except AlarmIterationDataError as error:
...
def _input_error(
    entry: AlarmExecutionEntry,
    iteration: AlarmExecutionIteration,
    error: AlarmIterationDataError,
) -> AlarmEvaluation:
    affected_inputs: tuple[AffectedInputIssue, ...] = ()
    if error.source_key is not None:
        affected_inputs = (
            AffectedInputIssue(
