# Compositor de navegación por secciones; la selección no cambia el contrato de la Rule.
# El editor monta sólo los controles de la sección activa y los callbacks existentes mantienen el autor.
...
RULE_SECTIONS = (
    ...
    ('evaluation', 'Evaluación y prioridad', 'Indica cómo se evalúa y cómo compite con otras reglas.'),
    ('reappearance', 'Reaparición', 'Opcional: cuándo puede volver una alarma previamente gestionada.'),
    ('deactivation', 'Desactivación', 'Configura si se permite una desactivación temporal.'),
    ('escalation', 'Origen y escalamiento', 'El escalamiento no es lo mismo que el destino visual.'),
                ...
                [
                    html.Div(
                        [
                            ...
                            html.Small('Completa cada sección. Los cambios se conservan en el editor.'),
                        ],
...
def _is_process_mode_control(value: object) -> bool:
    ...
    for child in children:
        component_id = getattr(child, 'id', None)
        if isinstance(component_id, dict) and component_id.get('field') == 'process_projection_mode':
            return True
    return False
