...
# Normaliza identidades contractuales ya validadas por Tool Structure.
...
# El wrapper de componente es determinista y único por Tool + component key.
def _component_wrapper_id(tool_key: str, component_key: str) -> str:
    return f'ada-tool-{tool_key}-component-{component_key}'


# El wrapper de subcomponente conserva siempre la identidad del owner canónico.
def _subcomponent_wrapper_id(
    ...
    return (
        f'ada-tool-{tool_key}-subcomponent-'
        f'{owner_component_key}-{subcomponent_key}'
    )


# Binding canónico de una zona interna pintable/identificable del Body.
...
class ToolBodySubcomponentBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if len(linked_component_keys) != len(set(linked_component_keys)):
            raise ToolBodyProjectionError(
                'Tool body linked component keys must be unique'
            )
    ...
    def visible_component_keys(self) -> tuple[str, ...]:
        # Una identidad compartida sigue siendo única, pero puede resolverse desde sus enlaces.
...
# Binding de una caja principal. No describe CSS ni contenido visual interno.
@dataclass(frozen=True, slots=True)
class ToolBodyComponentBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            not isinstance(item, ToolBodySubcomponentBinding)
        ...
        if any(item.owner_component_key != component_key for item in subcomponents):
            raise ToolBodyProjectionError(
                'Tool body subcomponent owner must match component key'
            )
...
# Proyección que expone identidades estables para una composición visual manual.
@dataclass(frozen=True, slots=True)
class ToolBodyProjection:
    ...
    def component_for_layout_role(
        ...
        # Process conserva LEFT/CENTER/RIGHT/BOTTOM como anclas estructurales.
        ...
        for component in self.components:
            if component.layout_role is role:
                return component
        raise ToolBodyProjectionError(
            f'Unknown Tool body layout role: {role.value!r}'
        )
    ...
    def subcomponent(
        ...
        *,
        ...
        # Un linked component resuelve la misma identidad canónica, nunca una copia.
