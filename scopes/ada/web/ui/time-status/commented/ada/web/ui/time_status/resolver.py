# Resolver temporal puro de las únicas fuentes con autoridad operacional en ADA Time Status.
...
# La clasificación no es extensible por configuración: las fuentes informativas nunca entran aquí.
...
def resolve_time_status_source_state(
    *,
    ...
    # Rechazar extras evita que BlockGrade u otra fuente futura adquiera autoridad de freshness por accidente.
    ...
    # Missing o datetime naive no permiten calcular una edad confiable y por eso son DATA_ERROR, no stale.
    ...
    # No se inventa una tolerancia de clock skew: un timestamp futuro es semánticamente inválido para este contrato.
    if timestamp > now:
        return TimeStatusSourceState(
            key=key,
            label=label,
...
def resolve_time_status_condition(
    *,
    ...
    # Los límites son inclusivos al entrar al nuevo estado: warning inicia PREVENTIVE y stale inicia HARD_STALE.
...
def format_time_status_relative_age(age_seconds: int) -> str:
    # Los buckets evitan ruido visual por segundos exactos y conservan el contrato recuperado del legacy.
    _require_age(age_seconds)
    if age_seconds < 10:
...
def _normalize_now(value: datetime | None) -> datetime:
    # now_utc explícito hace deterministas los tests; en producción se usa el reloj UTC real.
...
def _normalize_timestamp(value: datetime | None) -> datetime | None:
    ...
    # Un datetime naive es calidad temporal inválida; no se adivina timezone.
