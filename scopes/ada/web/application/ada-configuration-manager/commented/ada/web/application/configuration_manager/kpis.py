# Espejo pedagógico: integra el editor KPI con el workspace genérico del Manager.
...
# El editor KPI recibe un bridge de payload; no conoce SourceSnapshot ni contratos de publicación.
...
def create_kpi_manager_web_module(context: KpiManagerWebContext) -> WebModule:
    ...
    return WebModule(
        ...
        asset_layers=(*editor_module.asset_layers, KPI_MANAGER_ASSET_LAYER),
        register_callbacks=register_callbacks,
    )


# Guardar sólo actualiza el workspace del navegador; validar/publicar/proyectar sigue siendo responsabilidad del Manager.
def register_kpi_manager_callbacks(app: object, context: KpiManagerWebContext) -> None:
    @app.callback(
...
def _runtime_context(context: KpiManagerWebContext) -> object:
    return html.Section(
        [
            html.Div(
                [html.Span('Fuente de verdad'), html.Strong(context.source_name, id=KPI_SOURCE_NAME_ID)],
                className='ada-configuration-manager-kpis__runtime-source',
            ),
            html.Div(
                [html.Span('Proyección'), html.Strong(context.projection_name, id=KPI_PROJECTION_NAME_ID)],
                className='ada-configuration-manager-kpis__runtime-source',
            ),
            html.Div(
                [
                    ...
                    html.Span(
                        'Carga una configuración de KPI en el editor. No guarda, publica ni proyecta cambios.',
