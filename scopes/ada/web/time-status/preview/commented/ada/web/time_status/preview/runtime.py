# Este espejo documenta el harness TS-012 sin alterar su comportamiento.
# La aplicación productiva no se modifica: este módulo compone estados controlados para validación visual.
...
from atlanticus.web.models import ApplicationMetadata, WebApplicationDefinition, WebApplicationRuntime
...
# Compone la Generic Application real; el preview solo inyecta fixtures y callbacks de validación.
def create_preview_definition(*, tool_key: str = 'integrated_operations') -> WebApplicationDefinition:
...
# Materializa el runtime Atlanticus exactamente igual que una aplicación ADA normal.
...
# Construye estados deterministas para cada caso visual sin consultar Collector ni infraestructura externa.
...
# Mantiene el Header visualmente representativo usando tres indicadores reales del contrato actual.
...
# Completa la geometría del Header con un resumen de gestión de alarmas controlado.
...
# Identifica el slot real de Time Status y añade controles únicamente en el Body de validación.
...
# Los callbacks reemplazan solo el hijo del slot Time Status; el Header completo no se rerenderiza.
...
# Reutiliza el resolver productivo para no duplicar la semántica FRESH/PREVENTIVE/HARD_STALE/DATA_ERROR.
def _resolve_source(
    key: str,
    label: str,
...
# El Store es memory-only para que cada pestaña conserve una sesión visual independiente.
...
# El preview limita explícitamente los dos scopes que queremos comparar visualmente.
