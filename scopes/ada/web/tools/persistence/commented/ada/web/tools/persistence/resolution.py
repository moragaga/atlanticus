# Resuelve estado de Tool sin convertir ausencia o conectividad en una caída de la aplicación.
# Runtime puede leer Projection activa sin depender de Source.
# Los workflows que materializan desde Source usan una operación separada.
...
def resolve_active_tool_projection(
    ...
    *,
    ...
    try:
        # Runtime consulta sólo la Projection durable; Source no es una dependencia de lectura.
    ...
    if projection is None:
        # Store accesible sin projection es configuración todavía inexistente.
...
def project_current_tool_source(
    ...
    *,
    ...
    _validate_inputs(composition, source_key)
    try:
        # Esta operación sí pertenece al flujo Source -> Projection.
...
def _failure_resolution(error: BaseException) -> ToolProjectionResolution:
    # Sólo fallas de infraestructura conocidas se clasifican como UNAVAILABLE.
...
def _unavailable_resolution(error: BaseException) -> ToolProjectionResolution:
    # Diagnóstico visible, sin propagar la excepción al bootstrap de la Web.
    return ToolProjectionResolution(
