# Persiste la Tool Projection activa en filesystem bajo el namespace lógico de la Tool.
# El caller entrega un root de projections; este store no conoce SourceStore ni runtime.
...
class LocalToolProjectionStore(ProjectionStore[ToolConfiguration]):
    def __init__(self, settings: LocalToolProjectionStoreSettings) -> None:
        if not isinstance(settings, LocalToolProjectionStoreSettings):
            raise TypeError(
                'settings must be LocalToolProjectionStoreSettings'
            )
    ...
    def replace_active(
        ...
        try:
            # Un source_key tiene una sola projection activa dentro de este namespace Tool.
            payload = json.dumps(
    ...
    def _path(self, source_key: SourceKey) -> Path:
        # El hash sólo evita acoplar filenames a caracteres del SourceKey.
        # La separación entre Tools ya está garantizada por settings.root.
        digest = hashlib.sha256(
            source_key.value.encode('utf-8')
        ).hexdigest()
