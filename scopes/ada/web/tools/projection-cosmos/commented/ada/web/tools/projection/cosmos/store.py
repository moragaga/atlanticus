# Persiste Tool Projection en un container Cosmos compartido sin contaminar SourceKey.
# Cada store queda ligado a <application_namespace>/<tool_namespace> como partition key.
...
class CosmosToolProjectionStore(ProjectionStore[ToolConfiguration]):
    def __init__(
        ...
        *,
        ...
        if not isinstance(settings, CosmosToolProjectionStoreSettings):
            raise TypeError(
                'settings must be CosmosToolProjectionStoreSettings'
            )
        self._client = client
    ...
    def replace_active(
        ...
        # El provider agrega id/partition_key; ProjectionRecord conserva el SourceKey original.
...
def _cosmos_item_id(
    ...
    # La identidad física combina namespace de deployment y SourceKey de dominio.
