...
# Modela el snapshot autoritativo de KPI Configuration y la cobertura virtual DEFINED/MISSING sin persistir placeholders en Source.
    ...
    def __post_init__(self) -> None:
        ...
        if not revision:
            raise KpiDefinitionValidationError(
                'KPI configuration revision must not be empty'
            )
...
class KpiDefinitionCoverageItem:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.status, KpiDefinitionCoverageStatus):
            raise KpiDefinitionValidationError(
                'KPI definition coverage status is invalid'
            )
