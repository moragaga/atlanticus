# Codifica y publica releases inmutables del Registry.
# Este archivo es el espejo pedagógico del código productivo equivalente.

from __future__ import annotations

import gzip
import json
from dataclasses import dataclass
from io import BytesIO
from typing import Any

from ada.web.kpis.registry.models import KpiRegistry
from ada.web.kpis.registry.configuration.errors import KpiRegistrySourceError
from atlanticus.web.source.models import (
    ConcurrencyToken,
    HistoryPage,
    HistoryQuery,
    PublishRequest,
    PublishResult,
    SourceKey,
    SourceReleaseMetadata,
    SourceReleaseRef,
    SourceResource,
    SourceSnapshot,
)
from atlanticus.web.source.store import SourceStore

KPI_REGISTRY_SOURCE_DOCUMENT_TYPE = 'ada_kpi_registry_release'
KPI_REGISTRY_SOURCE_SCHEMA_VERSION = 1
KPI_REGISTRY_SOURCE_RESOURCE_PATH = 'kpis/registry.json.gz'
DEFAULT_MAX_COMPRESSED_BYTES = 5 * 1024 * 1024
DEFAULT_MAX_DECOMPRESSED_BYTES = 20 * 1024 * 1024


@dataclass(frozen=True, slots=True)
class KpiRegistrySourcePayload:
    registry: KpiRegistry
    published_by: str

    def __post_init__(self) -> None:
        if not isinstance(self.registry, KpiRegistry):
            raise KpiRegistrySourceError('KPI registry source payload is invalid')
        actor = self.published_by.strip() if isinstance(self.published_by, str) else ''
        if not actor:
            raise KpiRegistrySourceError('KPI registry publication actor must not be empty')
        object.__setattr__(self, 'published_by', actor)


@dataclass(frozen=True, slots=True)
class KpiRegistrySourceRelease:
    metadata: SourceReleaseMetadata
    payload: KpiRegistrySourcePayload

    @property
    def release_ref(self) -> SourceReleaseRef:
        return self.metadata.release_ref

    @property
    def registry(self) -> KpiRegistry:
        return self.payload.registry

    @property
    def published_by(self) -> str:
        return self.payload.published_by


class KpiRegistrySourceCodec:
    def encode(self, *, registry: KpiRegistry, published_by: str) -> SourceResource:
        payload = KpiRegistrySourcePayload(registry=registry, published_by=published_by)
        document = {
            'document_type': KPI_REGISTRY_SOURCE_DOCUMENT_TYPE,
            'schema_version': KPI_REGISTRY_SOURCE_SCHEMA_VERSION,
            'published_by': payload.published_by,
            'registry': payload.registry.to_document(),
        }
        raw = json.dumps(
            document,
            ensure_ascii=False,
            sort_keys=True,
            separators=(',', ':'),
        ).encode('utf-8')
        return SourceResource(
            logical_path=KPI_REGISTRY_SOURCE_RESOURCE_PATH,
            content=gzip.compress(raw, mtime=0),
        )

    def decode(self, resources: tuple[SourceResource, ...]) -> KpiRegistrySourcePayload:
        matches = tuple(
            resource
            for resource in resources
            if resource.logical_path == KPI_REGISTRY_SOURCE_RESOURCE_PATH
        )
        if len(matches) != 1:
            raise KpiRegistrySourceError(
                'KPI registry source release must contain exactly one registry resource'
            )
        document = _decode_document(matches[0].content)
        if document.get('document_type') != KPI_REGISTRY_SOURCE_DOCUMENT_TYPE:
            raise KpiRegistrySourceError('KPI registry source document type is invalid')
        if document.get('schema_version') != KPI_REGISTRY_SOURCE_SCHEMA_VERSION:
            raise KpiRegistrySourceError('KPI registry source schema version is invalid')
        try:
            registry = document['registry']
            if not isinstance(registry, dict):
                raise TypeError
            return KpiRegistrySourcePayload(
                registry=KpiRegistry.from_document(registry),
                published_by=str(document['published_by']),
            )
        except (KeyError, TypeError, ValueError) as error:
            raise KpiRegistrySourceError('KPI registry source contract is invalid') from error


class KpiRegistrySourceService:
    def __init__(
        self,
        *,
        source: SourceStore,
        source_key: SourceKey,
        codec: KpiRegistrySourceCodec | None = None,
    ) -> None:
        self._source = source
        self._source_key = source_key
        self._codec = codec or KpiRegistrySourceCodec()

    @property
    def source_key(self) -> SourceKey:
        return self._source_key

    def get_current(self) -> SourceSnapshot:
        return self._source.get_current(self._source_key)

    def load_current(self) -> KpiRegistrySourceRelease | None:
        snapshot = self.get_current()
        if snapshot.current is None:
            return None
        return self.load_release(snapshot.current.release_ref)

    def load_release(self, release_ref: SourceReleaseRef) -> KpiRegistrySourceRelease:
        metadata, resources = self._source.read_release(self._source_key, release_ref)
        if metadata.source_key != self._source_key:
            raise KpiRegistrySourceError('KPI registry source returned a different source key')
        if metadata.release_ref != release_ref:
            raise KpiRegistrySourceError('KPI registry source returned a different source release')
        return KpiRegistrySourceRelease(
            metadata=metadata,
            payload=self._codec.decode(resources),
        )

    def publish_registry(
        self,
        registry: KpiRegistry,
        *,
        published_by: str,
        expected_concurrency_token: ConcurrencyToken | None,
        basis_release: SourceReleaseRef | None,
    ) -> PublishResult:
        resource = self._codec.encode(registry=registry, published_by=published_by)
        return self._source.publish(
            PublishRequest(
                source_key=self._source_key,
                resources=(resource,),
                expected_concurrency_token=expected_concurrency_token,
                basis_release=basis_release,
            )
        )

    def query_history(self, *, page_size: int = 20, cursor: str | None = None) -> HistoryPage:
        return self._source.query_history(
            HistoryQuery(
                source_key=self._source_key,
                page_size=page_size,
                cursor=cursor,
            )
        )


def _decode_document(payload: bytes) -> dict[str, Any]:
    if len(payload) > DEFAULT_MAX_COMPRESSED_BYTES:
        raise KpiRegistrySourceError('KPI registry source compressed payload is too large')
    try:
        with gzip.GzipFile(fileobj=BytesIO(payload), mode='rb') as stream:
            raw = stream.read(DEFAULT_MAX_DECOMPRESSED_BYTES + 1)
    except OSError as error:
        raise KpiRegistrySourceError('KPI registry source payload is not valid gzip data') from error
    if len(raw) > DEFAULT_MAX_DECOMPRESSED_BYTES:
        raise KpiRegistrySourceError('KPI registry source decompressed payload is too large')
    try:
        document = json.loads(raw.decode('utf-8'))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise KpiRegistrySourceError('KPI registry source payload is not valid JSON') from error
    if not isinstance(document, dict):
        raise KpiRegistrySourceError('KPI registry source payload root must be an object')
    return document
