...
def test_service_selects_current_target_with_exact_tool_projection_dependency() -> None:
    ...
    service = create_kpi_projection_service(
        source=source,
...
def test_projection_persists_generic_dependency_and_configuration_payload() -> None:
    ...
    source_ref = release_ref('kpi-1')
    provider = DestinationProvider(destination_snapshot('tool-1'))
    resource = KpiSourceCodec().encode(configuration=configuration(), published_by='manager-user')
...
def test_projection_fails_if_tool_target_changes_after_target_selection() -> None:
    ...
    assert isinstance(error.value.__cause__, KpiConfigurationProjectionError)
    assert str(error.value.__cause__) == 'Tool projection changed before KPI projection'
...
def test_projection_builder_rejects_destination_missing_from_tool_catalog() -> None:
    ...
    with pytest.raises(
        KpiConfigurationProjectionError,
        match='Published KPI Configuration is not valid for projection',
    ):
        KpiProjectionBuilder(destinations=provider).build(
            target=target,
            release=release,
            resources=resources,
        )
...
def test_target_selection_requires_tool_projection() -> None:
    ...
    with pytest.raises(KpiConfigurationProjectionError, match='Tool projection is not available'):
