...
# La proyección schema v2 queda ligada a Source + KPI Configuration y materializa cobertura.
import hashlib
...
from ada.web.kpis.definition.errors import KpiDefinitionProjectionError
...
class KpiDefinitionProjection:
    ...
    def coverage_item(self, kpi_key: str) -> KpiDefinitionCoverageItem | None:
        ...
        if not normalized:
            raise KpiDefinitionProjectionError(
                'KPI definition coverage key must not be empty'
            )
        return next(
    ...
    def from_document(
        ...
        if document.get('document_type') != KPI_DEFINITION_PROJECTION_DOCUMENT_TYPE:
            raise KpiDefinitionProjectionError(
                'KPI definition projection document type is invalid'
            )
        ...
        try:
            ...
            return cls(
                ...
                coverage=tuple(
                    KpiDefinitionCoverageItem.from_document(item)
...
def _validate_coverage(
    ...
    if len(coverage_keys) != len(set(coverage_keys)):
        raise KpiDefinitionProjectionError(
            'KPI definition projection coverage keys must be unique'
        )
    definitions = {
        definition.kpi_key: definition
