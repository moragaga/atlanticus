...
def test_definition_normalizes_identity_and_field_names_without_changing_text() -> None:
    ...
    assert definition.fields == {'description': '  Material transportado  ', 'owner': None}
...
def test_definition_rejects_invalid_field_name_and_value() -> None:
    with pytest.raises(KpiDefinitionValidationError, match='KPI definition field name must be a non-empty string'):
        KpiDefinition(kpi_key='kpi', fields={' ': 'value'})
    with pytest.raises(KpiDefinitionValidationError, match='KPI definition field values must be strings or null'):
...
def test_definition_rejects_field_names_that_collide_after_normalization() -> None:
    with pytest.raises(KpiDefinitionValidationError, match='KPI definition field names must be unique'):
...
def test_definition_document_roundtrip_preserves_arbitrary_fields() -> None:
    ...
    assert KpiDefinition.from_document(definition.to_document()) == definition
