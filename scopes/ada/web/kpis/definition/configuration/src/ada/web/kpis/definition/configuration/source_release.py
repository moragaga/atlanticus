...
from ada.web.kpis.definition.configuration.errors import KpiDefinitionSourceError
...
KPI_DEFINITION_SOURCE_DOCUMENT_TYPE = 'ada_kpi_definition_release'
KPI_DEFINITION_SOURCE_SCHEMA_VERSION = 1
KPI_DEFINITION_SOURCE_RESOURCE_PATH = 'kpis/definition.json.gz'
DEFAULT_MAX_COMPRESSED_BYTES = 5 * 1024 * 1024
DEFAULT_MAX_DECOMPRESSED_BYTES = 20 * 1024 * 1024


@dataclass(frozen=True, slots=True)
class KpiDefinitionSourcePayload:
    configuration: KpiDefinitionConfiguration
    published_by: str

    def __post_init__(self) -> None:
        if not isinstance(self.configuration, KpiDefinitionConfiguration):
            raise KpiDefinitionSourceError('KPI Definition source configuration is invalid')
        actor = self.published_by.strip() if isinstance(self.published_by, str) else ''
        if not actor:
            raise KpiDefinitionSourceError('KPI Definition source publication actor must not be empty')
        object.__setattr__(self, 'published_by', actor)


@dataclass(frozen=True, slots=True)
class KpiDefinitionSourceRelease:
    metadata: SourceReleaseMetadata
    payload: KpiDefinitionSourcePayload
    ...
    @property
    def configuration(self) -> KpiDefinitionConfiguration:
...
class KpiDefinitionSourceCodec:
    def encode(
        self,
        *,
        configuration: KpiDefinitionConfiguration,
        ...
        return SourceResource(
            logical_path=KPI_DEFINITION_SOURCE_RESOURCE_PATH,
            content=gzip.compress(raw, mtime=0),
        )

    def decode(self, resources: tuple[SourceResource, ...]) -> KpiDefinitionSourcePayload:
        matches = tuple(
            resource
            for resource in resources
            if resource.logical_path == KPI_DEFINITION_SOURCE_RESOURCE_PATH
        )
        if len(matches) != 1:
            raise KpiDefinitionSourceError(
                'KPI Definition source release must contain exactly one configuration resource'
            )
        document = _decode_document(matches[0].content)
        if document.get('document_type') != KPI_DEFINITION_SOURCE_DOCUMENT_TYPE:
            raise KpiDefinitionSourceError('KPI Definition source release document type is invalid')
        if document.get('schema_version') != KPI_DEFINITION_SOURCE_SCHEMA_VERSION:
            raise KpiDefinitionSourceError('KPI Definition source release schema version is invalid')
        try:
            configuration = document['configuration']
            if not isinstance(configuration, dict):
                raise TypeError
            return KpiDefinitionSourcePayload(
                configuration=KpiDefinitionConfiguration.from_document(configuration),
                published_by=str(document['published_by']),
            )
        except (KeyError, TypeError, ValueError) as error:
            raise KpiDefinitionSourceError(
                'KPI Definition source release contract is invalid'
            ) from error


class KpiDefinitionSourceService:
    def __init__(
        self,
        *,
        source: SourceStore,
    ...
    def load_current(self) -> KpiDefinitionSourceRelease | None:
        snapshot = self.get_current()
        if snapshot.current is None:
            return None
        return self.load_release(snapshot.current.release_ref)

    def load_release(self, release_ref: SourceReleaseRef) -> KpiDefinitionSourceRelease:
        metadata, resources = self._source.read_release(self._source_key, release_ref)
        if metadata.source_key != self._source_key:
            raise KpiDefinitionSourceError(
                'KPI Definition source store returned a different source key'
            )
        if metadata.release_ref != release_ref:
            raise KpiDefinitionSourceError(
                'KPI Definition source store returned a different source release'
            )
        return KpiDefinitionSourceRelease(
            metadata=metadata,
...
def _decode_document(payload: bytes) -> dict[str, Any]:
    if len(payload) > DEFAULT_MAX_COMPRESSED_BYTES:
        raise KpiDefinitionSourceError(
            'KPI Definition source release compressed payload is too large'
        )
    try:
        ...
        raise KpiDefinitionSourceError(
            'KPI Definition source release payload is not valid gzip data'
        ) from error
    if len(raw) > DEFAULT_MAX_DECOMPRESSED_BYTES:
        raise KpiDefinitionSourceError(
            'KPI Definition source release decompressed payload is too large'
        )
    try:
        document = json.loads(raw.decode('utf-8'))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise KpiDefinitionSourceError(
            'KPI Definition source release payload is not valid JSON'
        ) from error
    if not isinstance(document, dict):
        raise KpiDefinitionSourceError(
            'KPI Definition source release payload root must be an object'
        )
    return document
