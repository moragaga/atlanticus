# Bootstrap de configuración local/Key Vault coherente con los demás procesos ADA.
...
from ada.processes.kpi_historian.settings import configuration_specs
...
def load_configuration(
    *,
    ...
    try:
        ...
        raise KpiHistorianConfigurationError(str(error)) from error
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise KpiHistorianConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise KpiHistorianConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise KpiHistorianConfigurationError('VOLUMEN_PATH must be an absolute path')
