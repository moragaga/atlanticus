    ...
    def read(self) -> KpiDeliveryConfiguration:
        ...
        if document is None:
            raise KpiTimeseriesDeliveryConfigurationError(
                'KPI delivery configuration was not found'
            )
...
def _configuration_from_document(
    ...
    *,
    ...
    if not isinstance(document, Mapping):
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration must be an object'
        )
    expected_fields = {
        'id',
        'partition_key',
    ...
    if document['id'] != expected_id:
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration id is invalid'
        )
    if document['partition_key'] != expected_partition_key:
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration partition_key is invalid'
        )
    if document['document_type'] != KPI_CONFIGURATION_DOCUMENT_TYPE:
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration document_type is invalid'
        )
    if document['schema_version'] != KPI_CONFIGURATION_SCHEMA_VERSION:
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration schema_version is invalid'
        )
    payload = document['configuration']
    if not isinstance(payload, Mapping) or set(payload) != {'bindings'}:
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration payload is invalid'
        )
    raw_bindings = payload['bindings']
    if not isinstance(raw_bindings, list):
        raise KpiTimeseriesDeliveryConfigurationError(
            'KPI delivery configuration bindings must be an array'
        )
    try:
        bindings = tuple(
        ...
        return KpiDeliveryConfiguration(
            ...
            tool_projection_revision=_optional_text(
                document['tool_projection_revision'],
                'tool_projection_revision',
            ),
            bindings=bindings,
        )
...
def _binding_from_payload(value: object, index: int) -> KpiDeliveryBinding:
    ...
    return KpiDeliveryBinding(
        ...
        latest_enabled=_required_bool(
            value['latest_enabled'], f'bindings[{index}].latest_enabled'
        ),
        series_enabled=_required_bool(
            value['series_enabled'], f'bindings[{index}].series_enabled'
        ),
