...
from ada.processes.kpi_runtime.settings import configuration_specs
...
def load_configuration(
    *,
    ...
    try:
        ...
        raise KpiRuntimeConfigurationError(str(error)) from error
...
def _required_bootstrap_value(values: Mapping[str, str], key: str) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise KpiRuntimeConfigurationError(f'{key} is required to resolve Key Vault')
    if value != value.strip():
        raise KpiRuntimeConfigurationError(f'{key} must not contain surrounding whitespace')
    return value
...
def _require_absolute_volume_path(configuration: ResolvedConfiguration) -> ResolvedConfiguration:
    ...
    if not configured_path.is_absolute():
        raise KpiRuntimeConfigurationError('VOLUMEN_PATH must be an absolute path')
...
def run(
    *,
    ...
    return build_composition(configuration=configuration, catalog=resolved_catalog).execute(
        argv=argv
    )
