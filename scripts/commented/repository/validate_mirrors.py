...
def _python_files(root: Path) -> dict[Path, Path]:
    # Indexa todos los módulos Python conservando su ruta relativa.
    return {path.relative_to(root): path for path in root.rglob('*.py')}


def _is_nonbehavioral_package_marker(path: Path) -> bool:
    # Un __init__.py auxiliar sólo se tolera si no ejecuta comportamiento.
    if path.name != '__init__.py':
        return False

    tree = ast.parse(path.read_text(encoding='utf-8'))
    body = list(tree.body)
    # Un docstring de paquete no se considera comportamiento ejecutable relevante.
...
def _validate_pair(source_root: Path, commented_root: Path) -> None:
    ...
    # La equivalencia se compara por AST para permitir comentarios adicionales.
...
def main(arguments: list[str]) -> int:
    ...
    # Los pares se procesan de forma independiente para reutilizar el helper
    # desde Web y, en el futuro, desde otras fronteras del repositorio.
