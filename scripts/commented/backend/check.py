...
# Cada capability representa un wheel estable del backend y concentra sólo
# la metadata que el gate necesita para probarlo de forma aislada o conjunta.
...
# El orden también es el orden legible de la capa. No expresa acoplamiento oculto:
# las dependencias reales siguen declaradas exclusivamente en cada pyproject.toml.
...
def _repository_root() -> Path:
    # El script vive en scripts/backend; dos niveles arriba está la raíz canónica.
...
def _run(command: list[str], *, cwd: Path) -> None:
    # Mostrar cada comando facilita reproducir un fallo sin esconder lógica en el gate.
...
def _resolve_capabilities(arguments: argparse.Namespace) -> tuple[BackendCapability, ...]:
    ...
    # Eliminar duplicados conserva el orden solicitado por el mantenedor.
...
def _validate_python_version() -> None:
    # El gate no acepta aproximaciones: la baseline productiva es CPython 3.14.2.
...
def _dependency_name(requirement: str) -> str:
    # Para correlación sólo necesitamos separar el nombre de su constraint o marker.
    for separator in ('==', '>=', '<=', '~=', '!=', '>', '<', ';', '['):
...
def _validate_internal_version_correlation(backend: Path) -> None:
    # Primero construimos la verdad de versiones declarada por cada wheel.
    ...
    # Luego exigimos pins exactos y correlacionados para toda dependencia interna del backend.
...
def _validate_mirrors(
    ...
    *,
    ...
    # Reutilizamos el validador transversal ya presente en la raíz, sin duplicar esa regla.
...
def _ruff_targets(
    ...
    *,
    ...
    # Ruff revisa sólo los módulos elegidos y el tooling que participa en este gate.
...
def _validate_lock(backend: Path) -> None:
    # uv.lock debe ser derivable del workspace actual; el gate nunca lo reescribe silenciosamente.
    _run(['uv', 'lock', '--check'], cwd=backend)


def _run_tests(capabilities: tuple[BackendCapability, ...], *, backend: Path) -> None:
    # Cada proyecto se prueba desde su propia raíz para evitar que la recolección conjunta
    # mezcle fragmentos del namespace ``atlanticus`` entre wheels hermanos.
    for capability in capabilities:
        project = backend / capability.project_root
        # Etiquetar cada ejecución permite localizar el package que falla sin inferirlo por orden.
...
def _build_wheels(capabilities: tuple[BackendCapability, ...], *, backend: Path) -> None:
    # La salida de build se recrea para que el conteo no pueda pasar por wheels antiguos.
    dist = backend / 'dist'
    if dist.exists():
        shutil.rmtree(dist)
    dist.mkdir()
