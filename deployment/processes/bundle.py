...
def main() -> None:
    ...
    process_roots = (
        tuple(
            resolve_process_root(repository_root, value, scope=arguments.scope)
            for value in arguments.processes
        )
        if arguments.processes
        else discover_processes(repository_root, scope=arguments.scope)
    )
