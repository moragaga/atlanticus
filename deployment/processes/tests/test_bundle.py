...
SPEC = importlib.util.spec_from_file_location('atlanticus_deployment_process_bundle', MODULE_PATH)
...
def test_repository_root_matches_deployment_tooling_location() -> None:
    ...
    assert (repository_root / 'deployment' / 'processes' / 'bundle.py').resolve() == MODULE_PATH.resolve()
...
def test_named_resolution_accepts_container_command_and_rejects_ambiguity(tmp_path: Path) -> None:
    repository = tmp_path / 'repository'
    operational = repository / 'scopes' / 'operational-data' / 'processes' / 'pi'
    ...
    assert resolve_process_root(repository, 'pi', scope='operational-data') == operational
...
def test_discover_projects_ignores_generated_runtime_and_artifacts(tmp_path: Path) -> None:
    repository = tmp_path / 'repository'
    ...
    _write_project(source, name='sample-process', command='sample', system_profile='base')
    _write_project(runtime, name='sample-process', command='sample', system_profile='base')
    _write_project(artifact, name='sample-process', command='sample', system_profile='base')
...
def test_internal_dependency_resolution_is_transitive_and_requires_exact_pins(tmp_path: Path) -> None:
    repository = tmp_path / 'repository'
    process = repository / 'scopes' / 'operational-data' / 'processes' / 'sample'
    ...
    dependencies = resolve_internal_dependencies(load_project(process), discover_projects(repository))
    ...
    with pytest.raises(ProcessBundleError, match='must be pinned exactly'):
        resolve_internal_dependencies(load_project(process), discover_projects(repository))
...
def test_bundle_uses_container_command_as_artifact_identity_and_excludes_local_files(
    ...
    (process / 'tests').mkdir()
    (process / 'tests' / 'test_sample.py').write_text('def test_sample():\n    assert True\n', encoding='utf-8')
...
def test_commented_bundle_is_structurally_equivalent() -> None:
    ...
    production = ast.dump(ast.parse((root / 'bundle.py').read_text(encoding='utf-8')), include_attributes=False)
    commented = ast.dump(
