...
# PREVIEW conserva el contrato operacional completo para diagnóstico local.
# EXPORT aplica la proyección remota WARNING+ para controlar costo y volumen.
...
def build_azure_observability_extension(
    *,
    ...
    preview_projection = OperationalEventProjection()
    if azure_settings.mode is AzureObservabilityMode.PREVIEW:
        ...
        try:
            ...
            return AzureObservabilityExtension(
                settings=azure_settings,
                sink=AzurePreviewSink(writer=writer, projection=preview_projection),
                trace_bridge=bridge,
            )
    ...
    export_projection = AzureProblemEventProjection()
    ...
    try:
        ...
        return AzureObservabilityExtension(
            settings=azure_settings,
            sink=AzureMonitorEventSink(projection=export_projection, backend=backend),
