...
def test_file_logs_switch_disables_only_file_persistence(tmp_path) -> None:
    settings = ObservabilitySettings.build(
        application='ada',
        service='web',
        environment='local',
    ...
    assert not resolve_observability_root(tmp_path, application='ada').exists()
    close_observability()
