...
def test_public_api_and_version_are_stable() -> None:
    assert key_vault.__version__ == '1.0.0'
...
def test_package_dependencies_match_the_agreed_boundary() -> None:
    ...
    assert 'atlanticus-kernel==1.0.0' in pyproject
...
def test_azure_local_integration_is_explicit_in_root_layer_gate() -> None:
    connectivity_root = _PACKAGE_ROOT.parent
    repository_root = connectivity_root.parent
    gate = (repository_root / 'scripts/connectivity/check.py').read_text(encoding='utf-8')
    check_sh = (repository_root / 'scripts/connectivity/check.sh').read_text(encoding='utf-8')
    compose = (connectivity_root / 'docker/azure-local/compose.yaml').read_text(encoding='utf-8')

    assert (
        "AZURE_LOCAL_CAPABILITIES = frozenset({'key-vault', 'storage', 'cosmos', 'redis'})" in gate
    )
    assert "'--azure-local'" in gate
    assert "compose_file = 'docker/azure-local/compose.yaml'" in gate
