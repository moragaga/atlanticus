...
class HttpResponse:
    ...
    def __post_init__(self) -> None:
        # Copiar y congelar impide que el transporte o el consumidor muten la respuesta.
    ...
    def decode_json(self) -> Any:
        ...
        try:
            # UTF-8 explícito evita que json.loads infiera otras codificaciones desde bytes.
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    # Un objeto con claves repetidas es ambiguo y no debe aplicar la política "última gana".
    result: dict[str, Any] = {}
...
def _parse_finite_float(value: str) -> float:
    # Un exponente válido sintácticamente también debe mantenerse dentro de float finito.
    parsed = float(value)
...
def _freeze_headers(value: Any) -> Mapping[str, str]:
    ...
    for name, header_value in value.items():
        # Se valida antes de normalizar para no ocultar un contrato inválido.
