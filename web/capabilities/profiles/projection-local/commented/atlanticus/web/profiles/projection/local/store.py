...
class LocalProfilesProjectionStoreSettings:
    # Root pertenece al entorno local; cada SourceKey obtiene un archivo determinístico separado.
    root: Path
...
class LocalProfilesProjectionStore(ProjectionStore[ProfileCatalog]):
    ...
    def replace_active(
        ...
        try:
            ...
            # El reemplazo atómico evita que otro lector observe un JSON parcialmente escrito.
    ...
    def _path(self, source_key: SourceKey) -> Path:
        # El hash evita convertir la SourceKey en una ruta o nombre de archivo controlado externamente.
