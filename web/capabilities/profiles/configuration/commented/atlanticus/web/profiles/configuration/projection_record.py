...
# El documento durable conserva provenance de Projection y sólo los perfiles configurados.
# basic/root/guest/local pertenecen al código de Profiles y se reconstruyen al crear ProfileCatalog.
def profiles_projection_to_document(
    ...
    *,
    ...
    # Estos campos sólo los necesitan stores como Cosmos; el contrato de dominio no depende de ellos.
...
# Al leer, se reconstituye un ProfileCatalog real; ningún consumer recibe ProfilesConfiguration.
def profiles_projection_from_document(
    ...
    try:
        ...
        if not isinstance(raw_profiles, list) or not all(
            ...
            raise TypeError
        # ProfileCatalog rechaza cualquier intento de redefinir los perfiles de sistema.
