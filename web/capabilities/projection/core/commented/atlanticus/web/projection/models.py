...
# Core no interpreta el payload de dominio; conserva únicamente su procedencia exacta.
...
# Un target identifica tanto la release propia como las proyecciones exactas de las que depende.
...
class ProjectionTarget:
    ...
    def __post_init__(self) -> None:
        # El orden no forma parte de la semántica, por eso se normaliza por SourceKey.
        object.__setattr__(
            self,
            'dependencies',
...
# La proyección activa conserva el target completo que realmente produjo el payload.
@dataclass(frozen=True, slots=True)
class ProjectionRecord(Generic[PayloadT]):
    ...
    def target(self) -> ProjectionTarget:
        # El target activo se obtiene sin consultar current ni reconstruir revisiones artificiales.
...
# Status conserva las releases genéricas vigentes y agrega la identidad exacta de dependencias.
@dataclass(frozen=True, slots=True)
class ProjectionStatus:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, 'current_dependencies', tuple(self.current_dependencies))
        object.__setattr__(self, 'projected_dependencies', tuple(self.projected_dependencies))
