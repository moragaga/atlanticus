...
def build_workspace_content(
    *,
    ...
    if workspace is None:
        # Incluso sin workspace el bloque conserva el mismo shell que el estado poblado.
        return html.Section(
            [
                _group_header(
                    'Trabajo local',
...
def _group_header(title: str, description: str) -> object:
    # Reutiliza las clases públicas de presentación del workflow sin importar helpers privados.
    return html.Header(
...
def _stage(
    ...
    # Mantiene la semántica de ManagerWorkspace y recupera la estructura que espera el CSS.
    return html.Article(
        [
            html.Header(
                [
