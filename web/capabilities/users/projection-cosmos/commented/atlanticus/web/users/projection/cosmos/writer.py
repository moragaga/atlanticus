...
# Estos valores forman parte del documento runtime ya consumido por UsersRuntimeStore.
...
# El writer recibe un cliente ya construido y declara solo las operaciones Cosmos que necesita.
# Así no posee secretos, provisioning ni el ciclo de vida de la conexión.
...
# Esta implementación materializa un snapshot de Source en users.runtime.
# La autoridad sigue siendo Users Configuration Source; Cosmos contiene estado derivado ejecutable.
class CosmosUsersRuntimeProjectionWriter(UsersRuntimeProjectionWriter):
    ...
    # Primero se retiran usuarios ausentes y después se materializan los presentes.
    # El orden favorece fail-closed ante un fallo parcial del snapshot.
    ...
    # Los resolved no locales que desaparecieron del Source se conservan como tombstones disabled.
    # No se borran porque una ausencia permitiría que observe() recreara un Pending/Guest.
    ...
    # Un usuario configurado se crea como resolved o promueve in-place un Pending con el mismo id.
    ...
    # Todo update existente usa ETag. Un conflicto solo se acepta si otro writer ya dejó
    # exactamente el estado funcional deseado; nunca se aplica last-write-wins ciego.
    def _patch_existing(
        self,
        *,
...
# Si una proyección distinta escribió después de comenzar este intento, este intento no puede
# retroceder el runtime. La siguiente ejecución desde el Source vigente reconciliará el snapshot.
...
# Actor y timestamp describen el intento, no la semántica funcional. Por eso un replay del
# mismo Source puede reconocer convergencia aunque esos metadatos difieran.
