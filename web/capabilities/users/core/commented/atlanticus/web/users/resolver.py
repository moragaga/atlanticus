...
# El resolver consume una única visión durable del ciclo de vida Users.
class UsersAccessResolver(AccessResolver):
    ...
    def resolve(self, identity: AuthenticatedIdentity, *, load_id: str) -> AccessDecision:
        try:
            # resolve sólo consulta. ABSENT exige observe para materializar Pending de forma atómica.
            record = self._store.resolve(identity)
            if record is None:
                # observe puede devolver Pending o un Managed concurrentemente promovido/deshabilitado.
            ...
            # Un fallo técnico o de identidad jamás se degrada silenciosamente a Guest.
...
# Tanto Pending como Managed deben seguir anclados exactamente a la identidad autenticada.
