...
# El documento persistido conserva provenance exacta y el payload propio de Navigation.
NAVIGATION_PROJECTION_DOCUMENT_TYPE = 'atlanticus_navigation_projection_record'
NAVIGATION_PROJECTION_SCHEMA_VERSION = 1


# El adapter Cosmos sólo necesita operaciones documentales neutrales; no depende del SDK de Azure.
...
# Local guarda un activo independiente por SourceKey y reemplaza el archivo de forma atómica.
class LocalNavigationProjectionStore(ProjectionStore[NavigationConfigurationCatalog]):
    ...
    def _path(self, source_key: SourceKey) -> Path:
        # El hash evita nombres de archivo inválidos y no altera la identidad lógica persistida.
...
# Cosmos usa un item determinista por SourceKey y el SourceKey como partition key.
